...
class Statement(Node):
    ...
    def Evaluate(self, symbolTable):
        for child in self.children:
            if symbolTable.get("return") is None:
...
class While(Node):
    ...
    def Evaluate(self, symbolTable):
        while (self.children[0].Evaluate(symbolTable).value or self.children[0].Evaluate(symbolTable).value != 0):
            if self.children[0].Evaluate(symbolTable).type == "String":
                raise SyntaxError(
                    f"INVALID OPERATION: type String is not a valid WHILE condition alone")
            else:
...
class FuncCall(Node):
    ...
    def Evaluate(self, symbolTable):
        ...
        returnVal = new_st.get("return")
        if returnVal != None:
            if func_type == returnVal.type:
                return new_st.get("return")
            else:
                raise Exception(
